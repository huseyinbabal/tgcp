...
pub async fn list_resources(
    ...
    let url = interpolate_url(&resource.api.base, &resource.api.path, client, extra.as_ref());
    ...
    let items = if resource.response_path.is_empty() {
        ...
        response.pointer(&format!("/{}", resource.response_path.replace('.', "/")))
...
pub async fn execute_action(
    ...
    let action = resource.actions.get(action_index)
        .ok_or_else(|| anyhow::anyhow!("Action index {} out of bounds", action_index))?;
    
    info!("Executing action '{}' on {}", action.display_name, resource.display_name);
    ...
    if let Some(zone_url) = item.get("zone").and_then(|v| v.as_str()) {
        ...
        if let Some(zone) = zone_url.split('/').last() {
    ...
    if let Some(region_url) = item.get("region").and_then(|v| v.as_str()) {
        if let Some(region) = region_url.split('/').last() {
...
fn interpolate_url(
    ...
    let full_path = format!("{}/{}", base.trim_end_matches('/'), path.trim_start_matches('/'));
